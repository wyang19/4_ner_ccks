...
def train(train_iter, test_iter, config):
    ...
    with graph.as_default():
        ...
        with session.as_default():
            ...
            cum_step = 0
            """
               多保存几个模型，再通过check_F1.py来查看每次训练得到的最高F1模型，取最优模型进行预测。
            """
...
def get_text_and_label(input_tokens_list, y_list):
    ...
    for i, input_tokens in enumerate(input_tokens_list):
        ...
        for index, num in enumerate(ys):
            ...
            #这是ccks的
            # if num in [3, 5, 7, 9, 11] and len(temp) == 0:
            #     temp.append(input_tokens[index])
            # elif num in [2, 4, 6, 8, 10] and len(temp) > 0:
            #     temp.append(input_tokens[index])
            # elif len(temp) > 0:
            #     label = "".join(temp)
            #     if len(set(label)) > 0:  # 干掉单字重复情况
            ...
            #             temp = []
             ...
             #这是深圳的数据
            if num in [3, 5, 7] and len(temp) == 0:
                temp.append(input_tokens[index])
            elif num in [2, 4, 6] and len(temp) > 0:
                temp.append(input_tokens[index])
            elif len(temp) > 0:
                label = "".join(temp)
                if len(set(label)) > 0:  # 干掉单字重复情况
